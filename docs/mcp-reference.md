...
On larger sources, the vector retriever's fetch is **approximate** rather than exhaustive: above a row-count threshold it uses a native quantized ANN index with a refine (over-fetch + exact re-score) pass, instead of scanning every row. This is a latency optimization, not a quality tradeoff — recall stays at ~99–100% versus an exhaustive scan, and reranking (when enabled) re-scores the retrieved candidates anyway. Small sources stay on exact search. See `SCRYBE_VECTOR_INDEX*` in [configuration.md](configuration.md) to tune or disable this.
...
- **Vector-only** (`SCRYBE_HYBRID=false`): `score` is **true cosine similarity** (`1 − cosine_distance`), typically `~0.7–0.9` for good matches, comparable across queries. Unaffected by whether the underlying fetch was exact or approximate (ANN) — the refine pass re-scores retained candidates exactly before this value is computed.
...
**Branch name resolution.** Scrybe accepts either short names (`dev`, `feat/example`) or qualified remote-tracking refs (`origin/dev`, `origin/feat/example`). The server resolves whichever form is actually stored — both HEAD and pinned branches are indexed under their plain short names (for a pinned branch the daemon reads content from the `origin/<branch>` ref but stores the index under the logical `<branch>` label). The `origin/`-qualified form is still accepted as a back-compat fallback for any legacy label. If neither form is indexed for a source, that source returns an empty result set. Set `SCRYBE_DEBUG_SEARCH=1` on the MCP server process to emit a debug line when a branch value cannot be resolved.
