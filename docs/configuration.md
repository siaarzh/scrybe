...
| `SCRYBE_DAEMON_SHUTDOWN_MAX_WAIT_MS` | `1800000` | Maximum time (ms, default 30min) a graceful shutdown waits for an in-flight reindex to finish before force-exiting. The idle shutdown timers also defer while a reindex is active, so a long index started from the CLI is no longer killed mid-job. Applies only to a user-requested `daemon stop` / signal — not to the daemon's own memory-ceiling restart (see `SCRYBE_DAEMON_RESTART_DRAIN_MS`). |
| `SCRYBE_DAEMON_RESTART_DRAIN_MS` | `2000` | Maximum time (ms) the daemon waits for in-flight work to finish when it restarts itself after crossing its memory ceiling. Deliberately short so an auto-restart frees memory and becomes reachable again within seconds instead of blocking on the 30-minute graceful-shutdown drain. |
| `SCRYBE_DAEMON_CGROUP_MAX_MB` | `4096` | Kernel-enforced memory ceiling (MB) applied when the daemon is started, on Linux with a reachable systemd user session. The kernel refuses further memory at request time instead of waiting for the in-process guard to notice. Set to `0` to disable. Keep it above `SCRYBE_DAEMON_MAX_RSS_HARD_MB` so an ordinary over-budget excursion still gets a clean self-restart rather than a kernel OOM kill; a misconfigured value is auto-adjusted upward with a warning rather than blocking daemon startup. |
| `SCRYBE_LOCAL_EMBED_TOKEN_BUDGET` | `4096` | Total padded token slots (rows x padded sequence length) allowed in one forward pass of the built-in offline embedding model. The model pads every text in a batch to the length of the longest one, so this — not the number of texts — is what sets peak memory. Lower it to trade indexing throughput for a lower ceiling. |
| `SCRYBE_PHASE_TELEMETRY` | `1` | Whether indexing writes per-phase memory and timing records to `phase-log.jsonl` in the data directory. Each record is written as its phase ends, so a job interrupted partway still leaves a trail. Set to `0` to disable. |
| `SCRYBE_PHASE_LOG_MAX_BYTES` | `16777216` | Size at which `phase-log.jsonl` rotates. |
| `SCRYBE_PHASE_LOG_BACKUPS` | `3` | How many rotated phase logs to keep. |
